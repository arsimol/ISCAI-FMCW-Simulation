...
M = 256;                % Αριθμός chirps (slow time)
...
% Δημιουργία των Range-Doppler Maps
rdm_A = generate_rdm(targets_A, M, N, Tc, lambda, c, mu, phase_code);
rdm_B = generate_rdm(targets_B, M, N, Tc, lambda, c, mu, phase_code);

% Καθορισμός των αξόνων (ίδιοι με αυτούς της plot_rdm)
range_axis = linspace(0, 120, N);
vel_axis = linspace(-25, 25, M);

% Εφαρμογή του CA-CFAR για τον εντοπισμό των στόχων
[det_R_A, det_V_A] = apply_2d_cfar(rdm_A, range_axis, vel_axis);
[det_R_B, det_V_B] = apply_2d_cfar(rdm_B, range_axis, vel_axis);

% Σχεδιασμός του Figure 2 με τα αποτελέσματα του CFAR
figure('Name', 'Figure 2: Range-Doppler Maps & CFAR Detection', 'Position', [100, 500, 800, 350]);

% Subplot (a)
subplot(1,2,1); 
plot_rdm(rdm_A, 'Fig 2(a) Well-separated targets', M, N);
hold on;
plot(det_R_A, det_V_A, 'ro', 'MarkerSize', 12, 'LineWidth', 2); % Κόκκινοι κύκλοι στους στόχους
legend('CFAR Detections', 'Location', 'northeast', 'Color', 'w');

% Subplot (b)
subplot(1,2,2); 
plot_rdm(rdm_B, 'Fig 2(b) Closely spaced targets', M, N);
hold on;
plot(det_R_B, det_V_B, 'ro', 'MarkerSize', 12, 'LineWidth', 2); % Κόκκινοι κύκλοι στους στόχους
legend('CFAR Detections', 'Location', 'northeast', 'Color', 'w');disp('Βήματα PC-FMCW (RDM & Sensing) ολοκληρώθηκαν.');

%% 4. ADB (Adaptive Driving Beam) - FIGURE 3 (ΥΨΗΛΗΣ ΑΚΡΙΒΕΙΑΣ)
car_w = 1;      % Πλάτος στόχου (m)
y_HL = 0.65;      % Θέση Αριστερού Προβολέα
y_HR = -0.65;     % Θέση Δεξιού Προβολέα

% -- Σενάριο 2(a): Oncoming Vehicle --
t_a = linspace(0, 7, 100); %6 second time stamp having 100 steps between 0->6 s
d_a = 150 - 20.5 * t_a;   %vertical range ,our lidar is able to see 150m ahead in this scenario the obgect travels 14.5 m/s      
y_target_a = -25 + 4 * t_a;  % horizontal range , the object moves closely to the start of the axis moving towards our vehicle at a speed of 3.3 m/s

%Because a MATLAB simulation does not have a real physical camera to look at a road and feed the θL and θR​ angles into the system, the code has to mathematically simulate what a camera would see.
...
figure('Name', 'Figure 2: ADB Shadow Angle Adjustments', 'Position', [100, 100, 900, 400]);
subplot(1, 2, 1);
...
% η συνάρτηη apply_2d_cfar δέχεται τον χάρτη RDΜ και τους άξονες για να
% ξέρει σε ποια μετρα ή ταχύτητες αντιστοιχεί το κάθε κελί 
function [det_ranges, det_vels] = apply_2d_cfar(rdm_db, range_axis, vel_axis)
    % 1. Μετατροπή από dB πίσω σε γραμμική κλίμακα ισχύος
    power_rdm = 10.^(rdm_db / 10); %ο υπολογισμός του cfar βασίζεται στον υπολογισμό του μέσου 
    %ορου της ενεργειας οπότε μετατρέπουμε τον πίνακα πίσω σε γραμμική κλίματα ισχύος  
   
    %βρίσκουμε τις διαστάσεις ενος πίνακα με Μ γραμμές για την ταχύτητα και
    %Ν στήλες για την εμβέλεια και τέλος φτιάχνουμε εναν κενό πίνακα στο
    %ίδιο μέγεθος (detections) όπου θα βάζουμε 1 όπου βρίσκουμε στόχο 
    
    [M, N] = size(power_rdm);
    detections = zeros(M, N);
    
    % 2. Παράμετροι 2D CA-CFAR
    %Tr ,Td training range & training doppler παίρνουμε 6 κελιά εκπαίδευσης
    %αυτά μετράνε τον θόρυβο 
    %Gr Gd guard range & guard doppler αφήνουμε 3 κελιά φρουράς γύρω απο το
    %κέντρο αυτα τα αγνοούμε για να μην προθέσει η ενέργεια του στόχου στον μέσο όρο του θορύβου  
    % alpha ο πολλαπλασιαστής του κατωφλίου .Ποσο πάνω απο τον μέσο θόρυβο
    % πρέπει να ειναι μία κορυφή για να θεωρηθεί στόχος 
    Tr = 6; Td = 6; % Μεγαλύτερο παράθυρο εκπαίδευσης για πιο σταθερό μέσο όρο
    Gr = 3; Gd = 3; % Μεγαλύτερο παράθυρο φρουράς για να μην μπερδεύεται από τα πλάγια του στόχου
    alpha = 80;     % ΔΡΑΣΤΙΚΗ ΑΥΞΗΣΗ: Κάνει το όριο πολύ πιο αυστηρό 
    
    % 3. Σάρωση RDM
    %αυτα ειναι τα loops που μετακινούν το κέντρο του παραθύρου σε κάθε
    %κελί ξεκινάνε απο το Td+Gd+1 γιατί απο εκεί χωράει να σχεδιαστεί
    %ολόκληρο το παράθυρο 
    for i = (Td+Gd+1) : (M-(Td+Gd)) %ο κώδικας δεν μπορει να σταματήσει μέχρι τα όρια τις δειγματοληψίας πρέπει να σταματήσουμε 9 γραμμές νωρίτερα για να χωράει το κάτω μέρος του παραθύρου 
        for j = (Tr+Gr+1) : (N-(Tr+Gr))
            % Απομόνωση ολόκληρου του παραθύρου γύρω από το κελί υπό δοκιμή (CUT)
            window = power_rdm(i-(Td+Gd):i+(Td+Gd), j-(Tr+Gr):j+(Tr+Gr));
            
            % Αφαίρεση της περιοχής φρουράς από τον υπολογισμό του θορύβου
           %βρίσκουν τη συνολική ισχύ των κελιών εκπαίδευσης αφαιρώντας τα
           %κελιά φρουράς 
            sum_noise = sum(window(:)) - sum(sum(power_rdm(i-Gd:i+Gd, j-Gr:j+Gr)));
           %υπολογισμός του ακριβούς πλήθους των κελιών εκπαίδευσης για να
           %ξέρουμε με τι νούμερο θα διαιρέσουμε για να βρούμε την μέση
           %ισχύ του θορύβου 
            num_training_cells = numel(window) - ((2*Gd+1)*(2*Gr+1));
            
            % Υπολογισμός μέσου θορύβου και κατωφλίου
            noise_avg = sum_noise / num_training_cells;
            threshold = alpha * noise_avg;
            
            % Το κελί υπό δοκιμή (Cell Under Test)
            CUT = power_rdm(i, j);
            %το παράθυρο σαρώνει τον πίνακα και εμείς απομονώνουμε την ισχύ
            %του ενός CUT 
            
            % ΕΛΕΓΧΟΣ 1: Είναι πάνω από τον θόρυβο;
            if CUT > threshold
                % ΕΛΕΓΧΟΣ 2 : Είναι το απόλυτο τοπικό μέγιστο (Peak);
                % Συγκρίνουμε το CUT με τα 8 αμέσως γειτονικά του κελιά.
                local_region = power_rdm(i-1:i+1, j-1:j+1);
                if CUT == max(local_region(:))
                    detections(i, j) = 1; % Επιβεβαιωμένος Στόχος!
                end
            end
        end
    end
    
    % 4. Αντιστοίχιση των θέσεων του πίνακα σε πραγματικά μέτρα και ταχύτητες
    [vel_idx, range_idx] = find(detections == 1);
    det_ranges = range_axis(range_idx);
    det_vels = vel_axis(vel_idx);
end